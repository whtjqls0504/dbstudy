/*
    ...
    3. 종류 
        ...
        4) MINUS     : 차집합, 첫 번째 SELECT 결과에서 두 번째 SEELCT 결과를 뺀다.
    4. 형식
        SELECT 칼럼1, 칼럼2
            FROM 테이블1
         집합연산자
...
-- 합집합 ) 사원 테이블과 부서 테이블에 존재하는 모든 부서번호를 조회하기 
...
-- 교집합) 사원 테이블과 부서 테이블 모두 존재하는 부서번호 조회하기 
...
-- 차집합 ) 부서 테이블에 존재하지만 사원 테이블에 존재하지 않는 부서번호 조회하기
-- (유령부서 조회하기)
SELECT DEPARTMENT_ID
  FROM DEPARTMENTS
 MINUS 
...
WITH MY_SUBQUERY(N, TOTAL) AS ( -- N, TOTAL은 MY_SUBQUERY의 칼럼을 의미한다.
        ...
        SELECT N + 1, TOTAL +(N + 1) -- N = N+1, TOTAL = TOTAL + (N + 1) 방식으로 반복해서 처리되는 부분
          FROM MY_SUBQUERY
         WHERE N < 10 -- 1~9까지는 사용이 가능하다.
) SELECT N,TOTAL FROM MY_SUBQUERY;


-- 활용2. WITH 문과 재귀 쿼리를 활용한 사원의 레벨 표시하기 
