...
INSERT INTO ORDER_T (ORDER_ID, CUST_ID, BOOK_ID, AMOUNT, ORDER_DT) VALUES(ORDER_SEQ.NEXTVAL, 1000, 3 ,2 , TO_DATE('20/07/03','YY/MM/DD'));
INSERT INTO ORDER_T (ORDER_ID, CUST_ID, BOOK_ID, AMOUNT, ORDER_DT) VALUES(ORDER_SEQ.NEXTVAL, 1001, 5 ,1 , TO_DATE('20/07/03','YY/MM/DD'));
INSERT INTO ORDER_T (ORDER_ID, CUST_ID, BOOK_ID, AMOUNT, ORDER_DT) VALUES(ORDER_SEQ.NEXTVAL, 1002, 6 ,2 , TO_DATE('20/07/04','YY/MM/DD'));
INSERT INTO ORDER_T (ORDER_ID, CUST_ID, BOOK_ID, AMOUNT, ORDER_DT) VALUES(ORDER_SEQ.NEXTVAL, 1003, 7 ,3 , TO_DATE('20/07/05','YY/MM/DD'));
INSERT INTO ORDER_T (ORDER_ID, CUST_ID, BOOK_ID, AMOUNT, ORDER_DT) VALUES(ORDER_SEQ.NEXTVAL, 1000, 2 ,5 , TO_DATE('20/07/07','YY/MM/DD'));
INSERT INTO ORDER_T (ORDER_ID, CUST_ID, BOOK_ID, AMOUNT, ORDER_DT) VALUES(ORDER_SEQ.NEXTVAL, 1003, 8 ,2 , TO_DATE('20/07/07','YY/MM/DD'));
INSERT INTO ORDER_T (ORDER_ID, CUST_ID, BOOK_ID, AMOUNT, ORDER_DT) VALUES(ORDER_SEQ.NEXTVAL, 1002, 10 ,2 , TO_DATE('20/07/08','YY/MM/DD'));
INSERT INTO ORDER_T (ORDER_ID, CUST_ID, BOOK_ID, AMOUNT, ORDER_DT) VALUES(ORDER_SEQ.NEXTVAL, 1001, 10 ,1 , TO_DATE('20/07/09','YY/MM/DD'));
INSERT INTO ORDER_T (ORDER_ID, CUST_ID, BOOK_ID, AMOUNT, ORDER_DT) VALUES(ORDER_SEQ.NEXTVAL, 1002, 6 ,4 , TO_DATE('20/07/10','YY/MM/DD'));
...
--낮은 가격순으로 RANK
SELECT * 
FROM (SELECT RANK() OVER(ORDER BY PRICE DESC) AS 순위
...
SELECT O.ORDER_ID               AS 구매번호
       ,C.CUST_NAME             AS 구매자
       ,B.BOOK_NAME             AS 책이름 
       ,B.PRICE * O.AMOUNT      AS 총구매액 -- 곱하기였기 때문에, 괄호나 SUM 등 통계함수를 구하지 않아도 됨.
       ,O.ORDER_DT              AS 주문일자
  FROM BOOK_T B INNER JOIN ORDER_T O
    ON O.BOOK_ID = B.BOOK_ID INNER JOIN CUSTOMER_T C
...
-- 15. '김연아'가 구매한 도서수를 조회하시오. >>도서수 >> COUNT, GROUP BY
-- 고객명  구매도서수
-- 김연아  2

SELECT C.CUST_NAME          AS 고객명
     , COUNT(O.BOOK_ID)    AS 도서구매수
...
SELECT C.CUST_NAME                  AS 고객명
     , COUNT(DISTINCT O.BOOK_ID)   AS 도서구매수
  FROM CUSTOMER_T C INNER JOIN ORDER_T O
    ON C.CUST_ID = O.CUST_ID
 WHERE C.CUST_NAME = '김연아'
 GROUP BY C.CUST_ID, C.CUST_NAME;
 
 -- 명시해야 마지막 순번인 SELECT 문이 실행된다. 
-- 고객 번호가 같아야 고객 이름과 같은 사람인 것. 

-- 16. 출판사별로 판매된 책의 갯수를 조회하시오. >> PUBLISHER, COUNT, GROUP BY
-- 출판사     판매된책수
-- 굿스포츠   4
-- 삼성당     0
-- 나이스북   3
...
-- 19. 모든 구매 고객의 이름과 총구매액(PRICE * AMOUNT)과 구매횟수를 조회하시오. 
-- 구매 이력이 없는 고객은 총구매액과 구매횟수를 0으로 조회하고, 고객번호 오름차순으로 정렬하시오.
-- 고객명  총구매액  구매횟수
-- 박지성  116000     3
-- 김연아  19000      2
-- 장미란  62000      3
