/*
    ROWID
    1. 오라클에서 제공하는 가상 칼럼(PSEUDO COULUMN)이다.
...
-- ROWID를 이용한 조회
-- 1. 오라클에서 가장 빠른 조회이다. 
-- 2. 실제로 사용가기가 불가능하다. 대신 인덱스를 사용한다. 
SELECT *
  FROM EMPLOYEES
...
-- 인덱스를 타는 조회 ( 인텍스 칼럼을 그대로 사용하지 않고 함수를 적용해서 사용하면 인덱스를 타지 않는다.)
SELECT *
  FROM EMPLOYEES
...
SELECT * FROM USER_INDEXES; --HR계정에 저장된 INDEX이다.

-- 인덱스가 설정된 칼럼 정보가 저장되어 있는 데이터 사전
-- DBA_IND_COLUMNS, USER_IND_COLUMNS, ALL_IND_COLUMN
SELECT * FROM USER_IND_COLUMNS;
SELECT * FROM USER_IND_COLUMNS WHERE TABLE_NAME ='EMPLOYEES';

-- 부서 테이블의 부서명(DEPARTMENT_NAME) 칼럼에 인덱스서럳
