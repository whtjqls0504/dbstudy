...
/*
    서버 메세지 출력하기
    1. 기본적으로 서버 메세지는 출력되지 않는다.
    2. 서버 메세지 출력을 위해서 최초 1회 아래 쿼리문을 실행한다.
        SET SERVEROUTPUT ON;
    3. 출력하는 방법
...
BEGIN
    ...
    WHILE TRUE LOOP
        IF TOTAL > 100 THEN
            EXIT;
        END IF;
        TOTAL := TOTAL + N; //N이 14일때 TOTAL이 끝났다. 
        N := N + 1;
    END LOOP;
    -- 결과 확인
...
-- 1부터 누적하기. 3의 배수는 제외하고 누적하기. 누적 결과는 100이 넘어가면 그만 누적하고 결과 출력하기.
DECLARE
    N NUMBER;
    TOTAL NUMBER;
    MODULAR NUMBER;
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        DBMS_OUTPUT.PUT_LINE(SQLERRM); -- 예외메세지
END;