/*
    ...
    2. 사용자 함수의 확인은 대부분 SELECT 문에서 이루어진다.
    3. 형식
        CREATE [OR REPLACE] FRUNCTION 함수명[(파라미터)]
        RETURN
        AS   --IS도 가능 
            [변수 선언]
        BEGIN
...
RETURN NUMBER  -- 반환타입은 NUMBER, VARCHAR2 처럼 크기 지정을 하지 않는다.
AS
...
WITH 
    ...
    RETURN VARCHAR2
    AS 
        DEPTNAME DEPARTMENTS.DEPARTMENT_NAME%TYPE; -- 결과
    BEGIN
