...
-- 1. 사원 테이블에서 동일한 부서번호를 가진 사원들을 그룹화하여 몇 명의 사원이 있는지 조회하시오.
SELECT DEPARTMENT_ID   -- GRUP BY  절에서 지정한 칼럼만 조회할 수 있다. 
    ,  COUNT(*) 
  FROM EMPLOYEES
 GROUP BY DEPARTMENT_ID;

-- 2. 사원 테이블에서 같은 직업을 가진 사원들을 그룹화 하여 각 그룹별로 연봉의 평균이 얼마인지 조회하시오
SELECT JOB_ID
     , ROUND(AVG(SALARY), 2)
  FROM EMPLOYEES
 GROUP BY JOB_ID;
...
SELECT DISTINCT DEPARTMENT_ID
  ...
  FROM EMPLOYEES; -- 동일한 부서 번호로 PARTITION BY로 분리하라
...
/* 
    ...
    3. GROUP BY 절이 필요하지 않는 조건은 WHERE절로 지정하면 된다. (HAVING -> GROUP BY 절 필수)
*/
...
-- 5. 사원 테이블에서 각 부서별 사원 수를 조회하시오. 단, 부서번호가 없는 사원은 제외하시오.
-- 조건 : 부서번호 IS NOT NULL
-- 조건에서 사용되는 부서번호는 GROUP BY 절이 필요없으르모 WHERE 절로 처리 
SELECT DEPARTMENT_ID
     , COUNT(*)
  FROM EMPLOYEES
